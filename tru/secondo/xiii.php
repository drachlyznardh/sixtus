<?php
	$d = $this->d;
	if ($this->addmeta ()) {
		$this->addtitle ('Dubbio', 'Ne vengono a tutti');
		$this->addprev ('Tru/Naluten/Vol.II/XII/', 'Conversazione');
		$this->addnext ('Tru/Naluten/Vol.II/XIV/', 'Proposta');
...
?><div class="small">
	<div class="section">
		...
		<div class="inside">
			<?=$d->speak ('simak', 'Siamo un mare di merda'); ?>
			<?=$d->speak ('lyz', 'Lo siamo?'); ?>
			<?=$d->speak ('simak', 'Decisamente e assolutamente'); ?>
		</div>
		<p>A volte le cose non vanno come vorrei.</p>
		<div class="inside">
			<?=$d->speak ('lyz', 'E perch&eacute; dovremmo essere nella merda?'); ?>
			<?=$d->speak ('simak', 'Ci, non ho voglia di discutere con l&apos;idiota: fallo tu'); ?>
			<?=$d->speak ('ci', 'Certo, Simak. Dunque, Lyz, come sai la reazione del soggetto `Jo` &egrave; stata diversa da quello che Simak si aspettava. La sua collaborazione &egrave; incerta e questo difatto &egrave; un ostacolo per il piano'); ?>
			<?=$d->speak ('lyz', 'Ah, gia, il piano... Posso ricordarvi che il piano &egrave; stupido e che un esercito non ci serve affatto? Tantomeno un esercito di mortali! Cazzo, Simak, ti basterebbe un fischio per schiodare dall&apos;Inferno legioni su legioni di... di... beh, di ogni genere di cosa!'); ?>
			<?=$d->speak ('simak', 'Quando sei stupido, stupido Lyz. So bene che posso richiamare tutto e tutti, dovunque; e so bene che non mi serve un esercito. Ma se ho scelto cos&igrave;, &egrave; perch&egrave; un buon motivo. Hai scordato anche quello? E dire ch&apos;ero io quello senza memoria...'); ?>
			<?=$d->speak ('lyz', 'Ehm... ?'); ?>
			<?=$d->speak ('simak', 'Ci, continua tu per me...'); ?>
			<?=$d->speak ('ci', 'Certamente. Come consuetudine, eLyznardh ha posto delle condizioni a se stesso per completare l&apos;impresa. Questa volta, inoltre, le limitazioni introdotte sono significativamente superiori a tutte le precedenti'); ?>
			<?=$d->speak ('ci', 'In particolare, eLyznardh ha imposto che &ndash; nonostante l&apos;invasione &ndash; non sia torto un capello ad alcuno, mortale o immortale, Chadan o Zathotan. Inoltre, eLyznardh ha comandato che Datundhum non subisca danni, e che Cheskaldhum non cada'); ?>
			<?=$d->speak ('lyz', 'Ah, ora ricordo. Tutte quella storia per cui dobbiamo portare la guerra all&apos;universo intero, senza rompere niente. Giusto?'); ?>
			<?=$d->speak ('ci', 'S&igrave;. Inoltre, eLyznardh ha scelto di non intervenire direttamente nei combattimenti'); ?>
			<?=$d->speak ('lyz', 'Si, me n&apos;ero dimenticato apposta, per evitare di dire nuovamente:'); ?>
			<?=$d->speak ('lyz forte', 'Ma che cazzo hai nella testa?!?!'); ?>
			<?=$d->speak ('lyz', 'Simak, tu sei il pi&ugrave; grosso idiota che io abbia mai conosciuto! Ma in fondo questo &egrave; il nostro modo di fare, giusto? eLyznardh &egrave; stupido perch&eacute; dentro ci siamo io e te, Simak! Ora, se permetti, mi dimenticher&ograve; nuovamente di questa faccenda. Per il tuo bene, spero che tu non me la faccia ricordare una seconda volta'); ?>
		</div>
		...
		<div class="inside">
			<?=$d->speak ('simak', 'Dunque, quali sono le nostre speranze attuali?'); ?>
			<?=$d->speak ('ci', 'Considerando le limitazioni autoimposte, e tenuto conto della tua magra figura di ieri, non molte. Se dovessi esprirermi formalmente, ti darei un [<span class="em">18.5%</span>]'); ?>
			<?=$d->speak ('simak', 'Se non m&apos;invento qualcosa, non verr&agrave; con noi...'); ?>
			<?=$d->speak ('ci', '&Egrave; improbabile che decida di seguirci, in base alla situazione attuale'); ?>
			<?=$d->speak ('ci', 'Il protocollo suggerirebbe semplicemente di guidare il suo futuro tagliando le altre vie possibili'); ?>
			<?=$d->speak ('simak', 'Ho scelto di non agire a quel modo, Ci. A quale scopo, infatti, obbligare questi mortali ad essere liberi? Gi&agrave; &egrave; accaduto in Zathot, e sai bene quanto &egrave; costato, a noi e a loro'); ?>
			<?=$d->speak ('simak', 'Se queste creature devono scorrazzare per il mio regno, allora che ne portino il peso. E non sar&ograve; io a guidare questa marmaglia, ma sar&agrave; uno di loro'); ?>
			<?=$d->speak ('ci', 'Il soggetto non &egrave; affatto dello stesso parere. L&apos;esercito, per come intendi realizzarlo, diverrebbe pericoloso sotto la sua guida'); ?>
			<?=$d->speak ('simak', 'E lo sarebbe anche di pi&ugrave; se li lasciassi senza una guida. Devo solo trovare un modo gentile affinch&egrave; lei scelga di accettare'); ?>
			<?=$d->speak ('lyz', 'Giochi fuori casa, caro il mio Simak. Fa&apos; come fanno i mortali, dunque'); ?>
			<?=$d->speak ('simak', 'E come fanno, i mortali? E come fanno cosa?'); ?>
			<?=$d->speak ('lyz', 'Andiamo, sai cosa intendo! I mortali hanno appreso da noi il modo di indirizzare la volont&agrave; altrui, e lo hanno fatto da tempo immemore'); ?>
			<?=$d->speak ('simak', 'Non ti seguo...'); ?>
			<?=$d->speak ('lyz', 'Dai, cazzone! Da quando esistono maschi e femmine, tutti gli esseri di Datundhum hanno giocherellato per portare un loro simile a costruire un nido assieme. Anche tu, per quanto differente da loro, hai fatto cose simili'); ?>
			<?=$d->speak ('simak', 'Stai proponendo un inganno, Lyz?'); ?>
			<?=$d->speak ('lyz', 'Suvvia, `inganno` &egrave; una parola meschina. Sto parlando di `suggerire` una via...'); ?>
			<?=$d->speak ('simak', 'Lyz, come puoi suggerire un&apos;idea del genere ad uno come me. Mi conosci bene, conviviamo nella stessa testa da innumerevoli eoni'); ?>
			<?=$d->speak ('lyz', 'Proprio perch&eacute; ti conosco bene ti sto spingendo verso una grande sfida'); ?>
			<?=$d->speak ('simak', 'C&apos;&eacute; differenza tra una grande sfida e una sfida impossibile. Credo di sapere dove stai puntando, e so che non pu&ograve; funzionare. Non sono fatto a quel modo!'); ?>
			<?=$d->speak ('lyz', 'Ah ah! Eccoti, dunque, signor <span class="em">&quot;solo Sacomne mi capisce&quot;</span>, eccoti rinunciare alla sfida'); ?>
			<?=$d->speak ('simak', '<span class="forte">Tu!</span> Tu mi stai dando del codardo?'); ?>
			<?=$d->speak ('lyz', 'Esatto, <span class="em">grande Simak</span>. Avrai anche grandi poteri, grande scienza, grandi passioni, ma sotto sotto rimani ancora quel piccolo insignificante Cheskal che si nasconde e resta indietro mentre i suoi fratelli minori imparano la vita prima di lui'); ?>
		</div>
		...
		<div class="inside">
			<?=$d->speak ('simak', '<span class="em">Quel</span> Simak &egrave; morto, morto per mano di eLyznardh'); ?>
			<?=$d->speak ('simak', 'Ed fui <span class="em">io</span> a guidare quella mano'); ?>
			<?=$d->speak ('simak', 'Ebbene, Datol Lyz, questa &egrave; una sfida, ed io la accetto. Poni le tue condizioni'); ?>
		</div>
		...
		<div class="inside">
			<?=$d->speak ('lyz', 'Ebbene, queste sono le condizioni che pongo per la sfida. Jo guider&agrave; il nostro esercito, ma giurer&agrave; fedelt&agrave; a Simak soltanto, poich&eacute; sar&agrave; Simak soltanto ad agire. Io giuro qui e subito di trattenermi da qualunque intromissione, fino al giorno in cui Jo assumer&agrave; il comando'); ?>
			<?=$d->speak ('ci', 'Ed io sottoscrivo e giuro lo stesso'); ?>
			<?=$d->speak ('simak', 'Ed io infine accetto queste condizioni, dunque'); ?>
			<?=$d->speak ('lyz', 'Non sia impaziente, bello, non ho ancora finito'); ?>
			<?=$d->speak ('simak', 'Ah no?'); ?>
			<?=$d->speak ('lyz', 'No, infatti: abbia dunque la cortesia di lasciarmi finire'); ?>
			<?=$d->speak ('simak', 'Prego, faccia, faccia pure'); ?>
			<?=$d->speak ('lyz', 'Orbene, dicevo... Ah, s&igrave;: sar&agrave; Simak soltanto, il Cheskald Simak'); ?>
			<?=$d->speak ('simak', 'Rinuncier&ograve; dunque ai miei titoli e ai miei poteri per tutto il tempo che sar&agrave; necessario'); ?>
			<?=$d->speak ('lyz', 'Fino al di lei giuramento, oppure &ndash; in caso di assoluta necessit&agrave; &ndash; fino a quando i tre presenti riterranno opportuno interrompere questo divieto'); ?>
			<?=$d->speak ('simak', 'Ebbene, &egrave; tutto?'); ?>
			<?=$d->speak ('lyz', '&Egrave; tutto'); ?>
			<?=$d->speak ('simak', 'Allora addio, signori. Ci rivedremo qui tra sette giorni, e quando torner&ograve; eLyznardh avr&agrave; un araldo che guider&agrave; i suoi eserciti'); ?>
			<?=$d->speak ('lyz', 'Sette giorni, eh? Buona fortuna, dunque'); ?>
			<?=$d->speak ('ci', 'Buona fortuna, <span class="em">Cheskald</span> Simak'); ?>
		</div>
		...
		<div class="outside">
			<?=$d->speak ('simadran', 'E per quanto scoprirsi d&apos;un tratto nudi e inermi, sperduti nel grande mondo sia brutta cosa, lasciamo per qualche tempo il piccolo grande Simak per occuparci invece di qualcuno che navi in acque ben pi&ugrave; minacciose'); ?>
			<?=$d->speak ('simadran', 'Trattasi di una certa giovane donna che abbiamo lasciato alla sua festa di compleanno, leggermente alterata &ndash; se cos&igrave; ci &egrave; concesso dire &ndash; ma non a sufficienza perch&eacute; qualche invitato potesse pensare a qualcosa di diverso dall&apos;eccitamento per la suddetta festa'); ?>
			<?=$d->speak ('simadran', 'Avevano lasciato cadere la povera ragazza tra le braccia del padre, il quale &ndash; mirabilmente guidato da una visione simile a quella di lei, ma molto pi&ugrave; positiva (non si dica che eLyznardh scelga male le visioni che invia) &ndash; cercava in ogni modo di raddrizzarle il morale, mentre procedeva goffamente a tentoni, con domande fin troppo vaghe, nel tentativo di comprendere la visione della figlia'); ?>
			<?=$d->speak ('simadran', 'A sua discolpa diremo che l&apos;amore paterno che dimostr&ograve; con abbracci e tentativi di compresione valse a ripagare la sua incapacit&agrave; di afferare la gravit&agrave; della situazione'); ?>
			<?=$d->speak ('simadran', 'Tale infatti fu l&apos;ardimento che impieg&ograve; per consolare la figlia che dimentic&ograve; di consolare se stesso ed arriv&ograve; a dimenticare il terribile annuncio ricevuto avuto dalla visione, e cio&egrave; che l&apos;indomani avrebbe dovuto salutare la sua figlia adorata per l&apos;ultima volta. Nella sua dimenticanza, accompagn&ograve; Jo, sfinita, nella sua stanza, l&apos;aiut&ograve; a coricarsi e poi si ritir&ograve; per abbandonarsi ad un sonno senza sogni'); ?>
		</div>
	</div><div class="section">
		<div class="outside">
			<?=$d->speak ('simadran', 'Ed ora che le creature mortali riposano, possiamo tornare da quell&apos;anima che non dorme ormai da alcuni secoli degli uomini'); ?>
			<?=$d->speak ('simadran', 'Quella creatura &egrave; perduta &ndash; per la prima volta da tempo, ma non per la prima volta &ndash; e cerca di ricordarsi del modo in cui si salv&ograve; quelle altre volte che si ritrov&ograve; in situazioni simili'); ?>
			<?=$d->speak ('simadran', 'Ahim&egrave;, non ci riusc&igrave; affatto. S&igrave;, perch&eacute; &ndash; per quanto fosse eLyznardh il pi&ugrave; grande e potenti di tutti i viventi del suo tempo, e per quanto egli fosse eLyznardh &ndash; c&apos;&eacute; da dire che Simak, il Cheskald Simak, da solo, non valeva poi un gran ch&eacute;... Ecco dunque quel che successe il mattino seguente, in quello stesso prato dove Jo vide la sua nuova casa per la prima volta'); ?>
		</div>
		...
		<div class="outside">
			<?=$d->speak ('lyznardh', 'Mi hai servito bene, Jona'); ?>
			<?=$d->speak ('lyznardh', 'Tutto quello che chiederai, ti verr&agrave; accordato sette volte'); ?>
			<?=$d->speak ('lyznardh', '<span class="em">Quel</span> giorno, se lo vorrai, siederai con me alla mia tavola'); ?>
		</div>
		...
		<div class="outside">
			<?=$d->speak ('lyznardh', 'Ebbene, Jo, lascia che mi presenti nuovamente, perch&eacute; ancora non mi conosci affatto'); ?>
		</div>
		...
		<div class="outside">
			<?=$d->speak ('simak', 'Piacere, io sono Simak!'); ?>
		</div>
