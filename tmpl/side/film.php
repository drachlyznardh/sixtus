<div class="section">
	<h2>
		Recensioni
	</h2><ol><li>
			<?=$d->link('Recensioni/Thor/','Thor')?> - 2011
		</li><li>
			<?=$d->link('Recensioni/AC-Brotherhood/','AC Brotherhood')?> - 2010
		</li><li>
			<?=$d->link('Recensioni/Robocop/','Robocop')?>
			<?=$d->link('Recensioni/Robocop/#II','1')?>,
			<?=$d->link('Recensioni/Robocop/#VI','2')?>  &amp;
			<?=$d->link('Recensioni/Robocop/#X','3')?> - '87,'90,'93
		</li><li>
			<?=$d->link('Recensioni/PaniPoniDash/','Pani Poni Dash')?>
		</li><li>
			<?=$d->link('Recensioni/BlassReiter/','BlassReiter')?>
		</li><li>
			<?=$d->link('Recensioni/Masterforce/','Masterforce')?>
		</li><li>
			<?=$d->link('Recensioni/DarkOfTheMoon/','Dark Of The Moon')?> – 2011
		</li><li>
			<?=$d->link('Recensioni/CapitanAmerica/','Capitan America')?> – 2011
		</li><li>
			<?=$d->link('Recensioni/AngelBeats/','Angel Beats!')?> – 2010
		</li><li>
			<?=$d->link('Recensioni/IPuffi/','I Puffi')?> – 2011
		</li><li>
			<?=$d->link('Recensioni/NessunDove/','Nessun Dove')?> – 1996
		</li><li>
			<?=$d->link('Recensioni/HarryPotter/','Harry Potter')?> – 1997~2007
		</li><li>
			<?=$d->link('Recensioni/KamenRider/','Kamen Rider')?> – 1971~2011
	</li></ol>
</div><div class="section">
	<h2>
		Work in Progress
	</h2><p>
		Harry Potter &amp; l'ordine della fenice
	</p><p>
		Pirate Sentai Gokaiger
	</p><p>
		Kamen Rider Fourze
	</p>
</div>
