<?php
	...
	$page['title'] = 'Condizioni che cambiano';
	$page['subtitle'] = 'Vecchie cose che tornano';
	
	$related['prev']['title'] = 'Storia IV';
	$related['prev']['request'] = 'Storie/IV/';
	$related['next']['title'] = 'Storia VI';
	$related['next']['request'] = 'Storie/VI/';
...
?>
	...
	<p>
		...
		intendo passarle. Eh, certo, perché io mi sono messo a scrivere la
		classe template proprio per non avere i tipi in mezzo alle balle,
		no?
	</p><p>
		E quindi, che succede in realtà? Semplicemente, anziché fare il
		copincollamento (copincollaggio forse? Boh, non è una vera parola)
		del codice, cambiando i tipi a mano, applico una sintassi lunga e
		brodosa, poi la metto a disposizione del compilatore, che a
		copincollare il codice ci pensa lui. Perché il codice template non
		viene riutilizzato, viene invece sfruttato per generare del codice.
		Fondamentalmente, quindi, quello che posso risparmiare è poco, ed è
		- ovviamente - ottenibile esclusivamente se mi metto a pensare nel
		modo che il C++ crede essere il meglio per me. 
	</p><p>
	...
	<p>
		...
		Ma di ché starò parlando? Torniamo indietro di alcuni anni, per
		capire come sono arrivato qui.
	</p><p>
		Mi è capitato un discreto numero di volte di partecipare ad attività
		sportive organizzate, anche a livello agonistico. Tutte cose che, a
		lungo andare, m'hanno stufato.
	</p><p>
		Ho fatto basket, nuoto, atletica, sci di fondo, pattinaggio, hockey,
		karate e ultimamente taiji (che, a quanto pare, si scrive così...).
		A parte queste, rimangono la corsa e il ciclismo, che mantengo
		tuttora per motivi che andrò a spiegare.
	</p><p>
		Man mano, tutte queste attività smettono di essere uno sport e un
		divertimento e diventano invece un impegno; allenamenti due volte
		alla settimana, anche tre; saltuari ma inprescindibili appuntamenti
		...
		al cinema. E l'unico motivo per cui io vado al cinema è per vedere
		il film.
	</p><p>
		E quindi, arriviamo alla parte triste. All'inizio di quest'estate
		due amici m'hanno proposto di partecipare con loro ed altra gente
		all'attività estiva del loro corso di taiji, che durante l'estate si
		vede chiudere in faccia la palestra (il Comune chiude le scuole,
		evidentemente); ergo, il gruppo si separa in quelli che vanno in
		ferie e in quei poveri studenti che non possono andarsene perché
		hanno esami da dare e preparare. Questi allegri studenti, due sere
	...
	<p>
		...
		Questo tale ha narrato la storia del suo apprendimento delle arti
		marziali, lasciando trasparire il proprio amore per esse e facendomi
		intuire la quantità di impegno necessaria per raggiungere il suo
		attuale livello. Ed ha parlato, tra le altre cose,
		dell'atteggiamento mentale che un praticante dovrebbe avere, o forse
		il modello verso cui dovrebbe tendere.
	</p><p>
		Domenica e lunedì sono andato a correre. Correndo, osservo le
		montagne e lascio la mente libera di vagare. Di tanto in tanto,
		mentre vado, realizzo alcune cose. Lunedì, mentre correvo, ho
		realizzato che l'idea di esercitare la mente e il corpo insieme non
		è quello che voglio fare. Quando corro, non voglio pensare
		attivamente, e non mi concentro su quello che faccio, ma penso
		cazzate. Mi ricordo vecchie barzellette e me le rido. Lascio che
		Codd mi benedica con qualche sprazzo di ottima programmazione.
	</p><p>
		A me non interessa allenarmi, migliorare le gambe o il fiato; a me
		piace correre e ci vado per due motivi. L'altro è sfogarmi.
	</p><p>
		E quando mi viene in mente una cazzata che vale la pena sentire, la
		dico. E se questo accade in un luogo dove comportarsi in questo modo
	...
	<p>
		...
		non poter superare, bensì d'avere davanti almeno cinquanta metri di
		stradina. Come ogni bravo bambino, quando ho davanti qualcosa di
		lento, m'accodo e rallento.
	</p><p>
		Una delle cose che fanno i ciclisti veloci è arrancare, quando si
		esaurisce lo sforzo e si rallenta. Il volume del mio respiro,
		immagino, fosse piuttosto alto al momento. La prova fu che, per ben
		due volte, la ragazza ciclista davanti a me tenta di voltarsi a
		guardare ma subito torna a fissare la strada. E non era per saper
		dov'andava. Perché, ovviamente, io non me ne vado in giro con un
		sorriso.
	</p><p>
		E la strada è ancora stretta, quindi arriviamo in fondo lei davanti
		ed io che la seguo. Il punto in cui la pista ciclabile si interrompe
		per l'attraversamento pedonale è anche più stretto, quindi devo
		lasciare che lei attraversi per prima. Solo che anche dall'altro
		lato delle striscie pedonali la strada è strettissima, quindi ancora
		una volta mi tocca inseguire.
	</p><p>
		Giunta dall'altra parte, la ragazza si volta completamente e mi
		fissa. Ed io penso: &laquo;Adesso le faccio un salutino per
		calmarla, magari&raquo; ma mentre m'avvicino lei si volta e nasconde
		il volto.
	</p><p>
		Non so descrivere come mi son sentito, ma non è stato bello. Temo di
		...
		è ben puntata verso il basso, sicché mi rallenta ma non illumina,
		motivo per cui la tengo spenta. E poi, io ci vedo, al buio.
	</p><p>
